...
**Meshes:** Quest Enable mesh Read/Write (readable copies on the clone; the FBX stays as it is). Merge skinned meshes that animate together. Merge identical material slots. Allow shuffling material slots. Disable Update When Offscreen (off). Disable receive shadows (off). Those two also stay on Scene for a project write.
